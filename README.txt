...
online accounts using the HTTPs method. Briefly, the ATD app requires
a client connection to a PHP server that contains the ATD code. The
client can then submit the credentials that identify the ATD for
company-specific use to the server, which then directs the client to
the Quickbooks site, where the client logs into their account and
selects the company they want to retrieve the report from. Upon
logging in, the client is then brought back to the server’s home
page, where they can download the ATD reports. ATD can retrieve a
list of all transactions from the client’s company organized by class
or in a class-agnostic manner to account for transactions that have
not been registered to a specific class yet. Companies that use
quickbooks online will often assign each transaction to a
specific “class”, but the current API code provided by Quickbooks
does not include the class ID  or name for transactions obtained
using the General Ledger report function. Therefore, ATD was
developed to retrieve the General Ledger report from each class,
ultimately providing the client with transactions organized by class.
Please refer to the Diagram for more information on the ATD
process. 
...
4) Here you will be provided with Development Settings, which provide
...
2) A list of to-do items will pop up. Fulfill all of these tasks in in
order to retrieve your company credentials. These tasks include
...
ADT, adjustments and additions to the code may need to be made. 
...
that your browser is redirected to after logging in with ATD. the
redirect URI listed here should match the redirect in the ATD code.
...
locally, and connect on your web server locally. To use this current
...
Credentials page. To create your own redirect URI website, you can
use the  atd_uri files as a template, then upload to the file to a
...
atd_uri.php files. It redirects the ATD client to the ATD server to
execute the callback procedure which obtains the access code for the
company. The current URI is set so that you can host the ATD server
locally, and connect as a client locally. Instructions on how to
separate the ATD client from the ATD server are at the end of this
file.

3) Home URI: This URI can be user dependent, and it must be set in the
atd_uri.php file. It is the same as the callback URI with no call to
a php file.  The Home URI is used at the end of the callback and
report procedures to redirect the ATD client to the home page. he
current URI is set so that you can host the ATD server locally, and
...
address OR localhost>:<port to listen on>(ex: php -S localhosti
wo:3000)
